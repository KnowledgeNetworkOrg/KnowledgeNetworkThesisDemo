...
import { relationsOfNode } from './corpusrelations'
...
// the map (#341, OB-238), and the decomposed relations index followed them to
// `./corpusrelations` when the document pane grew its Relations rail (#342) — two hosts read
// each of them now. `relationsOfNode` is still MODULE SCOPE there, and that is load-bearing:
// the split pane memoises its answers on the function's identity, and a fresh closure per
// render would recompute the whole via-children roll-up on every keystroke in the filter box.
